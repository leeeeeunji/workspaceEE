...
public class LifeCycleCounterServlet extends HttpServlet {
	...
	public LifeCycleCounterServlet() {
		System.out.println("0. LifeCycleCounterServlet() 기본 생성자 호출[최초 요청 시, 단 한번 호출] 객체 주소 : " + this);
	}
	
	@Override
		public void init(ServletConfig config) throws ServletException {
			super.init(config);
			System.out.println("1. init()메소드 -> 생성자 호출 직후 단 한번 호출[객체초기화, 리소스획득]");
		}
	...
	@Override
		public void destroy() {
			System.out.println("3. distroy()메소드 -> 서블릿객체가 메모리에서 해제되기 직전에 호출[리소스 반납]");
		}
