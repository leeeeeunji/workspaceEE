...
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			Properties controllerMappingProperties = new Properties(); //Properties = Map
			...
			*/
			...
			System.out.println("-----설정파일[" + configFile + "]을 이용하여 Controller객체 생성-----");
			while (commandKeyIterator.hasNext()) {
				 ...
				 * Controller클래스 이름을 사용해서 Controller객체 생성
				 * 	1. Controller클래스 이름을 사용해서 클래스를 메모리에 로딩
				 * 	2. 메모리에 로딩된 클래스의 기본생성자를 호출하여 객체 생성
				 */
		...
		handlerMapping.put("/guest_list.do", new GuestMainController());
		handlerMapping.put("/guest_view.do", new GuestMainController());
		handlerMapping.put("/guest_write_form.do", new GuestMainController());
		handlerMapping.put("/guest_write_action.do", new GuestMainController());
		handlerMapping.put("/guest_modify_form.do", new GuestMainController());
		handlerMapping.put("/guest_modify_action.do", new GuestMainController());
		handlerMapping.put("/guest_remove_action.do", new GuestMainController());
		handlerMapping.put("/guest_error.do", new GuestMainController());
	...
	private void processRequest(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 2-1.DispatcherServlet이 클라이언트 요청에 따른 업무를 실행 할 Controller객체 얻기
