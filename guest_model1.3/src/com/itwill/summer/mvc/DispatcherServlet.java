...
/*
 * 1. 클라이언트(웹브라우져)의 모든 요청을 받는 서블릿 작성(front Controller)
 * 2. 확장자가 *.do인 모든 클라이언트의 요청이 서블릿을 실행하도록 web.xml에 url pattern mapping
   << web.xml >>
    <servlet>
...
public class DispatcherServlet extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 1.DispatcherServlet이 클라이언트의 요청URI를 사용해서 요청을 분석.
		 */
		 ...
		 * 2-3. DispatcherServlet이 controller객체의 handleRequest메소드의 실행 반환 값인
		 * 		forwardPath를 받는다.
		 */
