...
public class UserDao {
	...
	public int insert(User user) throws Exception {
		...
		try {
			...
			/*
			 * 예외발생과 관계없이 반드시 실행되는 코드(특히 connection 무조건 종료되어야 함)
			 */
			if (pstmt != null) { //null여부 확인 안 하면 NullPointException 발생
				pstmt.close();
			}
			if (con != null) {
				con.close();
	...
	/*
	 * 사용자 아이디에 해당하는 정보를 데이타베이스에서 찾아서 User 도메인클래스에 저장하여 반환
	 */
	public User findUser(String userId) throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		User findUser = null; //null = 해당 user정보 존재하지 않는다
		try {
			con = dataSource.getConnection();
	...
	/*
	 * 모든 사용자 정보를 데이타베이스에서 찾아서 List<User> 콜렉션에 저장하여 반환
	 */
	...
	/*
	 * 인자로 전달되는 아이디를 가지는 사용자가 존재하는 지의 여부를 판별
	 */
